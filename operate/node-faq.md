...
Our system that checks stake, creates and send the cNFT (shdwOperator NFT), and assigns the Discord role take a few minutes to process. It is designed to be scalable and thus orderly queues database, Solana RPC, and Discord API calls with retries for robustness. Please be patient and give the cranks ample time to process your verification. 
